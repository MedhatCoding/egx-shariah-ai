    ...
    for name, symbol in SHARIAH_STOCKS.items():
        try:
            ...
            if hist.empty or len(hist) < 60:
                continue
            # قفزة يومية شاذة (>35%) غالباً خطأ split أو بيانات تالفة - نستبعد السهم كاملاً
            if hist["Close"].pct_change().abs().max() > 0.35:
                continue
...
def main():
    ...
    for name, data in stocks_data.items():
        ...
        # فحص اتساق السعر: فرق كبير (>15%) بين آخر إغلاق تاريخي وسعر السوق الحالي غالباً
        # معناه خطأ تعديل تجزئة سهم (split) في المصدر - نستبعد السهم كاملاً بدل عرض رقم خاطئ.
    ...
    if price_mismatch_excluded:
        print(f"🚫 تم استبعاد {len(price_mismatch_excluded)} سهم لتعارض في السعر (احتمال خطأ split): {', '.join(price_mismatch_excluded)}")
    ...
    if price_mismatch_excluded:
        header += f"\n🚫 تم استبعاد {len(price_mismatch_excluded)} سهم لتعارض بيانات السعر (احتمال split)."
    if market_trend:
