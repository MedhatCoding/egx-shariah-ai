...
يسجل في Google Sheets ويتابع الأهداف/وقف الخسارة زي daily_alert_job.py الأصلي.
...
def score_reversal_opportunity(ind):
    score, tags = 0, []
    if ind["rsi14"] < 35:
        score += 2; tags.append(f"تشبع بيعي ({ind['rsi14']:.0f})")
    elif ind["rsi14"] < 45:
        score += 1
...
def fetch_fundamentals(symbol):
    try:
        ...
        return {"available": available, "pe": pe, "roe": roe, "debt_to_equity": debt_to_equity, "profit_margin": profit_margin}
    except Exception:
        return {"available": False, "pe": None, "roe": None, "debt_to_equity": None, "profit_margin": None}
...
def get_live_price(symbol):
    ...
    يرجع None بأمان لو المصدر ده مش متاح أو فشل، بدون أي بيانات وهمية.
    """
    try:
...
def main():
    ...
    # --- تحديث سعر الفرص المختارة فقط (momentum + reversal) بالسعر اللحظي لو متوفر ---
    # بنعمل ده للفرص المختارة بس (مش كل 113 سهم) عشان نوفر وقت ولا نستهلك حدود الطلبات
    live_price_note = False
    for c in candidates + reversal_candidates:
        symbol_full = c["symbol"] + ".CA"
        live_price = get_live_price(symbol_full)
        if live_price and abs(live_price - c["ind"]["last_close"]) / c["ind"]["last_close"] > 0.002:
            c["ind"] = dict(c["ind"])  # نسخة مستقلة عشان منعدلش القاموس المشترك في current_prices
            c["ind"]["last_close"] = live_price
            live_price_note = True
    ...
    if live_price_note:
        header += "\n💡 بعض أسعار الفرص أدناه مُحدَّثة لحظياً (مصدر أحدث من الشمعة اليومية)."
    if market_trend:
