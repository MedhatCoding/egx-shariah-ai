...
# --- 7.6 سجل التوصيات (Journal) - لتتبع الأداء الفعلي بمرور الوقت ---
LOG_PATH = "recommendations_log.csv"
LOG_COLUMNS = ["التاريخ", "السهم", "الرمز", "سعر الدخول", "الهدف", "وقف الخسارة", "النقاط", "سبب الدخول", "نُفّذت فعلاً؟", "سعر التنفيذ الفعلي"]
        ...
        try:
            ...
            for c in LOG_COLUMNS:
                if c not in df.columns:
                    df[c] = "" if c not in ("نُفّذت فعلاً؟",) else False
            return df
        except Exception:
            return pd.DataFrame(columns=LOG_COLUMNS)
    return pd.DataFrame(columns=LOG_COLUMNS)

def save_execution(idx, executed, actual_entry):
    """يحدّث حالة التنفيذ وسعر الدخول الفعلي لتوصية معينة في السجل."""
    log_df = load_log()
    if idx in log_df.index:
        log_df.loc[idx, "نُفّذت فعلاً؟"] = bool(executed)
        log_df.loc[idx, "سعر التنفيذ الفعلي"] = actual_entry if executed else ""
        try:
...
def evaluate_log(log_df, current_prices: dict):
    ...
    for _, row in log_df.iterrows():
        ...
        elif current >= row["الهدف"]:
            statuses.append("✅ تحقق الهدف")
        elif current <= row["وقف الخسارة"]:
...
if log_df.empty:
    ...
    for idx, row in evaluated_sorted.iterrows():
        with st.container(border=True):
            st.markdown(f"**{row['السهم']}** ({row['الرمز']}) — {row['التاريخ']} | {row['الحالة الحالية']} | {row['النقاط']} نقطة")
            ...
            if st.button("💾 حفظ", key=f"save_{idx}"):
                save_execution(idx, executed, actual_entry if executed else None)
                st.success("تم الحفظ ✅")
                st.rerun()
