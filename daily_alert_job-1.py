...
def send_telegram_message(text):
    """
    تم إصلاحها: بتطبع كود الحالة ونص رسالة الخطأ الكاملة الراجعة من تليجرام عند أي فشل،
    بدل ما تكتفي بطباعة الاستثناء العام. ده اللي هيوضحلنا السبب الحقيقي وراء 400 Bad Request.
    """
    ...
    try:
        ...
        if r.status_code != 200:
            print(f"⚠️ فشل إرسال تليجرام - كود {r.status_code}. رد تليجرام الكامل: {r.text}")
            return False
        return True
    except Exception as e:
        print("⚠️ فشل إرسال تليجرام (استثناء غير متوقع):", e)
        return False
...
def check_hits_and_stops(ws, current_prices: dict):
    ...
    تم إصلاحها: بنعد التنبيهات اللي نجحت فعلياً بس (مش كل اللي حاولنا نبعتها).
    """
    if ws is None:
        return
    try:
    ...
    print(f"🔔 تم إرسال {sent_count} من أصل {len(rows_to_update)} تنبيه هدف/وقف خسارة بنجاح فعلي.")
...
def main():
    ...
    current_prices = {}
    failed = []
    latest_trading_date = None  # آخر يوم تداول فعلي حسب بيانات yfinance نفسها، وليس تاريخ تشغيل السيرفر
    ...
    ws = get_worksheet()
    append_new_candidates_to_log(ws, candidates, latest_trading_date or datetime.now().strftime("%Y-%m-%d"))

    # افحص التوصيات القديمة في السجل: هل وصلت هدفها أو ضربت وقفها؟ (وابعت تنبيه فوري لو كده)
    check_hits_and_stops(ws, current_prices)

    # ابعت ملخص تليجرام يومي بالفرص الجديدة - بتاريخ آخر يوم تداول فعلي، مش تاريخ تشغيل السيرفر
    report_date = latest_trading_date or datetime.now().strftime("%Y-%m-%d")
    if not candidates:
        msg = f"📅 آخر يوم تداول: {report_date}\nمفيش فرص وصلت لعتبة {SCORE_THRESHOLD} نقطة."
    else:
        lines = [f"📊 <b>ملخص الفرص - آخر يوم تداول {report_date}</b>", f"({len(candidates)} فرصة بعتبة {SCORE_THRESHOLD}+ نقطة)\n"]
    ...
    if not ok:
        print("⚠️ فشل إرسال ملخص اليوم - راجع رسالة الخطأ التفصيلية أعلاه لمعرفة السبب.")
