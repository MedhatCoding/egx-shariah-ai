...
def get_live_price(symbol):
    """
    يحاول جلب أحدث سعر متاح من مصدر لحظي منفصل (fast_info) بدل الاعتماد فقط على آخر
    شمعة يومية في .history() اللي ممكن تكون متأخرة يوم كامل لأسهم EGX تحديداً.
    كاش قصير (3 دقائق) عشان يفضل التطبيق سريع مع تحديث معقول. يرجع None بأمان لو فشل.
    """
    try:
...
# تحديث سعر الفرص المختارة فقط (مش كل 113 سهم) بالسعر اللحظي لو متوفر ومختلف بشكل ملموس
live_price_used = False
for c in candidates:
    live_price = get_live_price(c["symbol"] + ".CA")
    if live_price and abs(live_price - c["ind"]["last_close"]) / c["ind"]["last_close"] > 0.002:
        c["ind"] = dict(c["ind"])
        c["ind"]["last_close"] = live_price
        live_price_used = True
if live_price_used:
    st.caption("💡 بعض الأسعار أدناه مُحدَّثة لحظياً (مصدر أحدث من آخر شمعة يومية مسجّلة).")
...
for a in reversal_candidates:
    live_price = get_live_price(a["symbol"] + ".CA")
    if live_price and abs(live_price - a["ind"]["last_close"]) / a["ind"]["last_close"] > 0.002:
        a["ind"] = dict(a["ind"])
        a["ind"]["last_close"] = live_price
