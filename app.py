...
def fetch_all_data():
    ...
    أي سهم لا تتوفر له بيانات كافية، أو تحتوي سلسلته على قفزة يومية شاذة (>60% في يوم واحد -
    غالباً خطأ split أو بيانات تالفة يفسد حساب كل المؤشرات لشهور بعدها)، يتم تجاهله تماماً.
    ...
    for name, symbol in SHARIAH_STOCKS.items():
        try:
            ...
            if hist.empty or len(hist) < 60:
                failed.append(name)
                continue
            daily_pct_changes = hist["Close"].pct_change().abs()
            if daily_pct_changes.max() > 0.6:
                failed.append(name)
                continue
...
def compute_indicator_series(hist: pd.DataFrame):
    """
    نسخة متجهة (vectorized) من compute_indicators: بتحسب كل المؤشرات لكل يوم في السلسلة
    دفعة واحدة، بدل إعادة الحساب من الصفر عند كل نقطة زمنية في الـ backtest.
    هذا هو سبب البطء الأساسي سابقاً - نفس النتائج الرياضية، لكن أسرع بعشرات المرات.
    """
    ...
    minus_di = 100 * (minus_dm.rolling(14).mean() / tr_smooth.replace(0, np.nan))
    dx = 100 * (plus_di - minus_di).abs() / (plus_di + minus_di).replace(0, np.nan)
    adx14 = dx.rolling(14).mean().fillna(20.0)

    momentum_5d = close.pct_change(5) * 100
    momentum_20d = close.pct_change(20) * 100

    avg_vol20 = volume.rolling(20).mean()
    vol_ratio = (volume / avg_vol20.replace(0, np.nan)).fillna(1.0)
    avg_daily_value = avg_vol20 * close

    rolling_high_150 = high.rolling(150, min_periods=1).max()
    pct_from_high = ((close - rolling_high_150) / rolling_high_150) * 100

    try:
        weekly_close = close.resample("W").last().dropna()
        weekly_sma4 = weekly_close.rolling(4).mean()
        weekly_confirmed_weekly = weekly_close > weekly_sma4
        weekly_confirmed = weekly_confirmed_weekly.reindex(close.index, method="ffill").fillna(False)
    except Exception:
        weekly_confirmed = pd.Series(False, index=close.index)

    day_change_pct = close.pct_change() * 100

    return pd.DataFrame({
        "last_close": close, "day_change_pct": day_change_pct, "sma20": sma20, "sma50": sma50, "rsi14": rsi14, "atr14": atr14,
        "adx14": adx14, "momentum_5d": momentum_5d, "momentum_20d": momentum_20d,
        "vol_ratio": vol_ratio, "pct_from_high": pct_from_high, "avg_daily_value": avg_daily_value,
        "weekly_confirmed": weekly_confirmed,
    })
...
def score_opportunity(ind):
    ...
    # انخفاض حاد في يوم واحد - علامة تحذير قوية حتى لو الزخم الأسبوعي موجب (ممكن يكون خبر سلبي طارئ)
    if ind.get("day_change_pct", 0) < -10:
        score -= 3
        tags.append(f"⚠️ انخفاض حاد اليوم ({ind['day_change_pct']:.1f}%) - احتمال خبر سلبي، توخَّ الحذر الشديد")
...
def score_reversal_opportunity(ind):
    ...
    شرط إجباري: لازم يكون فيه تشبع بيعي فعلي (RSI<45) وإلا السهم مش فرصة ارتداد أصلاً،
    مهما كانت باقي عوامل الزخم/السيولة قوية (لأنها كانت بتخلي أسهم متشبعة شرائياً تظهر هنا غلط).
    """
    if ind["rsi14"] >= 45:
        return 0, []
...
def fetch_fundamentals(symbol):
    ...
    try:
        ...
        return {"available": False, "pe": None, "roe": None, "debt_to_equity": None, "profit_margin": None, "sector": "غير مصنّف", "market_price": None}
...
def backtest_stock(hist, score_threshold, strategy="momentum", holding_days=10, atr_target_mult=2, atr_stop_mult=1, step=1):
    """
    ...
    يحسب كل المؤشرات مرة واحدة بشكل متجه (compute_indicator_series) بدل إعادة حسابها
    في كل نقطة زمنية - نفس النتائج الرياضية، أسرع بعشرات المرات من النسخة القديمة.
    هذا ليس ضماناً للمستقبل، لكنه قياس صادق لأداء القاعدة على بيانات فعلية سابقة.
    """
    ...
    series_df = compute_indicator_series(hist)
    ...
    for i in range(50, n - holding_days, step):
        ...
        if pd.isna(row["sma50"]) or pd.isna(row["rsi14"]) or pd.isna(row["atr14"]):
            continue
        ind = {
            "last_close": float(row["last_close"]), "day_change_pct": float(row["day_change_pct"]) if not pd.isna(row["day_change_pct"]) else 0.0,
            "sma20": float(row["sma20"]), "sma50": float(row["sma50"]),
            "rsi14": float(row["rsi14"]), "atr14": float(row["atr14"]), "adx14": float(row["adx14"]),
            "momentum_5d": float(row["momentum_5d"]) if not pd.isna(row["momentum_5d"]) else 0.0,
            "vol_ratio": float(row["vol_ratio"]), "pct_from_high": float(row["pct_from_high"]),
            "weekly_confirmed": bool(row["weekly_confirmed"]),
        }
        ...
        for j in range(i + 1, min(i + 1 + holding_days, n)):
            if high_vals[j] >= target:
                outcome, exit_price = "win", target
                break
            if low_vals[j] <= stop:
                outcome, exit_price = "loss", stop
                break
        if outcome is None and i + 1 < n:
            exit_price = float(close_vals[min(i + holding_days, n - 1)])
...
# --- 7.6 سجل التوصيات (Journal) - في Google Sheets الموحّد (دائم، مشترك مع سكريبت تليجرام) ---
LOG_COLUMNS = ["التاريخ", "السهم", "الرمز", "سعر الدخول", "الهدف", "وقف الخسارة", "النقاط", "سبب الدخول", "نُفّذت فعلاً؟", "سعر التنفيذ الفعلي", "المصدر"]

def get_gsheet_worksheet():
    """
    يتصل بنفس شيت Google Sheets اللي يستخدمه سكريبت تليجرام (daily_alert_job.py)،
    عشان يبقى سجل واحد موحّد بدل ملفين منفصلين. يحتاج GSHEET_URL و GCP_SERVICE_ACCOUNT_JSON
    في إعدادات Secrets الخاصة بتطبيق Streamlit (نفس القيم المستخدمة في GitHub Secrets).
    """
    try:
        gsheet_url = st.secrets.get("GSHEET_URL") or os.getenv("GSHEET_URL")
        creds_raw = st.secrets.get("GCP_SERVICE_ACCOUNT_JSON") or os.getenv("GCP_SERVICE_ACCOUNT_JSON")
        if not gsheet_url or not creds_raw:
            return None
        import gspread
    ...
    try:
        ...
        for c in LOG_COLUMNS:
            if c not in df.columns:
                df[c] = "" if c != "نُفّذت فعلاً؟" else False
        return df
    except Exception:
        return pd.DataFrame(columns=LOG_COLUMNS)

def save_execution(ws, idx, executed, actual_entry):
    """يحدّث حالة التنفيذ وسعر الدخول الفعلي لتوصية معينة مباشرة في Google Sheets."""
    if ws is None:
        return
    try:
        notified_col = LOG_COLUMNS.index("نُفّذت فعلاً؟") + 1
        entry_col = LOG_COLUMNS.index("سعر التنفيذ الفعلي") + 1
        row_num = idx + 2  # +2: صف العناوين + الفهرسة تبدأ من 1 في جوجل شيت
        ws.update_cell(row_num, notified_col, bool(executed))
        ws.update_cell(row_num, entry_col, actual_entry if executed else "")
    except Exception:
        pass

def append_to_log(ws, existing_df, candidates, reasons=None):
    if ws is None or not candidates:
        return
    reasons = reasons or {}
    existing = set(zip(existing_df["التاريخ"], existing_df["السهم"])) if not existing_df.empty else set()
    ...
    for c in candidates:
        ...
        new_rows.append([
            ...
            c["total_score"], reasons.get(c["name"], " | ".join(c["tags"]))[:200],
            False, "", "التطبيق",
        ])
    if new_rows:
        try:
...
def evaluate_log(log_df, current_prices: dict):
    if log_df.empty:
        return log_df
    statuses = []
    for _, row in log_df.iterrows():
        ...
        try:
            ...
            statuses.append("⏳ لا تتوفر بيانات حالية")
            continue
        if current is None:
            statuses.append("⏳ لا تتوفر بيانات حالية")
        elif current >= target:
            statuses.append("✅ تحقق الهدف")
        elif current <= stop:
            statuses.append("❌ ضرب وقف الخسارة")
        else:
            statuses.append("⏳ لسه مفتوحة")
    log_df = log_df.copy()
...
"""
    try:
        ...
        # fallback: نعرض العلامات المحسوبة فعلياً لكل سهم (نفس التاغز المستخدمة في التسجيل)
        # بدل جملة ثابتة كانت ممكن تفوّت تحذيرات مهمة زي الانخفاض الحاد في يوم واحد.
        explanation_map = {}
        for c in candidates:
            tags = c.get("tags", [])
            explanation_map[c["name"]] = " | ".join(tags) if tags else "لا توجد ملاحظات فنية بارزة حالياً."
        return explanation_map
...
⚠️ هذا التطبيق أداة مساعدة للتحليل الفني فقط وليس توصية استثمارية مضمونة. البيانات مصدرها Yahoo Finance وقد تحمل تأخيراً حقيقياً (لوحظ أحياناً حتى يوم كامل لبعض أسهم البورصة المصرية، وليس دقائق فقط). <strong>تحقق دائماً من السعر الفعلي عبر تطبيق الوساطة الخاص بك قبل تنفيذ أي صفقة</strong> — لا تعتمد على أي سعر هنا للتنفيذ المباشر.
</div>
""", unsafe_allow_html=True)
...
duration_choice = st.selectbox("المدة:", ["يوم واحد", "أسبوع (5 جلسات)", "شهر (21 جلسة)"], label_visibility="collapsed")
duration_days = {"يوم واحد": 1, "أسبوع (5 جلسات)": 5, "شهر (21 جلسة)": 21}[duration_choice]
...
candidates = [a for a in analyzed if a["score"] >= score_threshold]
...
# (الأسهم "غير مصنّف" مستثناة من القيد لأن معندهاش بيانات قطاع حقيقية من Yahoo أصلاً)
diversified = []
sector_counts = {}
for a in candidates:
    sec = a["sector"]
    if sec != "غير مصنّف" and sector_counts.get(sec, 0) >= 2:
        continue
...
journal_ws = get_gsheet_worksheet()
...
if not candidates:
    ...
    # تسجيل التوصيات الحالية في السجل الموحّد (Google Sheets) تلقائياً مع سبب الدخول
    append_to_log(journal_ws, load_log(journal_ws), candidates, reasons=explanations)
...
# 2.6 كل الأسهم مرتبة (قائمتان كاملتان: صعود وارتداد) - نقاط محسوبة رياضياً بدون شرح AI لكل سهم
st.subheader("📋 كل الأسهم مرتبة تنازلياً حسب النقاط")
st.caption("ترتيب كل الـ113 سهم (المتوفر لها بيانات) بنقاطها الفنية، من غير حد أدنى للعتبة أو فلاتر سيولة/تنويع. الشرح النصي بالذكاء الاصطناعي مقصور على أفضل 8 فرص أعلاه فقط لتوفير الوقت وحدود الاستخدام المجانية.")

col_full_up, col_full_down = st.columns(2)

with col_full_up:
    with st.expander(f"📈 ترتيب فرص الصعود (زخم) — {len(analyzed)} سهم", expanded=False):
        ranked_up = sorted(analyzed, key=lambda x: x["total_score"], reverse=True)
        for i, a in enumerate(ranked_up, 1):
            ind = a["ind"]
            top_tags = " | ".join(a["tags"][:2]) if a["tags"] else "لا توجد إشارات بارزة"
            change_class = "price-up" if ind["day_change_pct"] >= 0 else "price-down"
            st.markdown(f"""
            <div class="stock-card">
                <div style="display:flex; justify-content:space-between;">
                    <span class="stock-title">{i}. {a['name']} ({a['symbol']})</span>
                    <span class="badge-score">{a['total_score']} نقطة</span>
                </div>
                <div style="display:flex; justify-content:space-between; margin-top:4px;">
                    <span style="font-size:0.85rem; color:#64748b;">{top_tags}</span>
                    <span class="{change_class}" style="font-size:0.85rem;">{ind['last_close']:.2f} ({ind['day_change_pct']:+.1f}%)</span>
                </div>
            </div>
            """, unsafe_allow_html=True)

with col_full_down:
    with st.expander(f"🔄 ترتيب فرص الارتداد (تشبع بيعي) — {len(analyzed)} سهم", expanded=False):
        ranked_down = sorted(analyzed, key=lambda x: x["reversal_score"], reverse=True)
        for i, a in enumerate(ranked_down, 1):
            ind = a["ind"]
            top_tags = " | ".join(a["reversal_tags"][:2]) if a["reversal_tags"] else "لا يوجد تشبع بيعي حالياً"
            change_class = "price-up" if ind["day_change_pct"] >= 0 else "price-down"
            st.markdown(f"""
            <div class="stock-card">
                <div style="display:flex; justify-content:space-between;">
...
quality_candidates = [a for a in analyzed if a["fund"]["available"] and a["fund_score"] >= 3]
...
log_df = load_log(journal_ws)
if journal_ws is None:
    st.warning("⚠️ لم يتم الاتصال بسجل Google Sheets. تأكد من إضافة GSHEET_URL و GCP_SERVICE_ACCOUNT_JSON في إعدادات Secrets الخاصة بالتطبيق.")
elif log_df.empty:
    ...
    evaluated = evaluate_log(log_df, current_prices)
    evaluated_sorted = evaluated.sort_values(by="التاريخ", ascending=False)
    ...
    wins_count = (evaluated_sorted["الحالة الحالية"] == "✅ تحقق الهدف").sum()
    losses_count = (evaluated_sorted["الحالة الحالية"] == "❌ ضرب وقف الخسارة").sum()
    closed = wins_count + losses_count
    if closed > 0:
        actual_wr = (wins_count / closed) * 100
        st.markdown(f"""
        <div class="data-note">
        📈 <strong>نسبة النجاح الفعلية المسجلة حتى الآن:</strong> {actual_wr:.0f}% ({wins_count} ربح / {losses_count} خسارة من أصل {closed} توصية مغلقة).
        هذا هو الرقم الأهم لأنه أداؤك الحقيقي، وليس محاكاة.
        </div>
        """, unsafe_allow_html=True)

    for idx, row in evaluated_sorted.iterrows():
        with st.container(border=True):
            source_tag = f" | 📍 {row.get('المصدر', 'التطبيق')}" if row.get("المصدر") else ""
            st.markdown(f"**{row['السهم']}** ({row['الرمز']}) — {row['التاريخ']} | {row['الحالة الحالية']} | {row['النقاط']} نقطة{source_tag}")

            col1, col2, col3 = st.columns(3)
            col1.metric("سعر الدخول الموصى به", f"{row['سعر الدخول']} EGP")
            col2.metric("الهدف", f"{row['الهدف']} EGP")
            col3.metric("وقف الخسارة", f"{row['وقف الخسارة']} EGP")

            executed_prev = str(row.get("نُفّذت فعلاً؟", "")).strip().lower() in ("true", "1", "yes", "نعم")
            try:
                entry_prev_val = float(row.get("سعر التنفيذ الفعلي")) if str(row.get("سعر التنفيذ الفعلي", "")).strip() not in ("", "nan", "None") else float(row["سعر الدخول"])
            except (ValueError, TypeError):
                entry_prev_val = float(row["سعر الدخول"])

            executed = st.checkbox("✅ نفّذت الصفقة دي فعلاً؟", value=executed_prev, key=f"exec_{idx}")
            actual_entry = float(row["سعر الدخول"])
            if executed:
                actual_entry = st.number_input("دخلت بكام فعلياً؟ (EGP)", min_value=0.0, value=entry_prev_val, step=0.05, key=f"entry_{idx}")
                if actual_entry > 0:
                    target = float(row["الهدف"])
                    profit_if_target = ((target - actual_entry) / actual_entry) * 100
                    st.markdown(f"💰 **نسبة الربح لو تحقق الهدف:** {profit_if_target:+.1f}%")
                    current_price = current_prices.get(row["السهم"])
                    if current_price is not None:
                        current_pl = ((current_price - actual_entry) / actual_entry) * 100
                        pl_color = "#16a34a" if current_pl >= 0 else "#dc2626"
                        st.markdown(f"📊 **الربح/الخسارة الفعلي الحالي:** <span style='color:{pl_color}; font-weight:bold;'>{current_pl:+.1f}%</span>", unsafe_allow_html=True)

            if st.button("💾 حفظ", key=f"save_{idx}"):
                save_execution(journal_ws, idx, executed, actual_entry if executed else None)
                st.success("تم الحفظ ✅")
                st.rerun()
    ...
    st.caption("💾 السجل محفوظ بشكل دائم في Google Sheets - لن يُمسح عند تحديث التطبيق.")

st.caption(f"آخر تحديث للبيانات: {datetime.now().strftime('%Y-%m-%d %H:%M')} | المصدر: Yahoo Finance (بيانات حقيقية، تأخير طبيعي محتمل)")
