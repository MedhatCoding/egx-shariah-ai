...
def fetch_all_data():
    ...
    أي سهم لا تتوفر له بيانات كافية، أو تحتوي سلسلته على قفزة يومية شاذة (>35% في يوم واحد -
...
for name, data in stocks_data.items():
    ...
    # فرق كبير (>15%) غالباً معناه خطأ تعديل تجزئة سهم (stock split) في المصدر - نستبعد السهم
...
if journal_ws is None:
    ...
    st.caption("⚠️ ملاحظة: السجل مخزّن على خادم التطبيق وقد يُمسح عند إعادة نشر التطبيق. حمّل نسخة CSV بشكل دوري للاحتفاظ بها.")
