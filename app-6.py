...
analyzed = []
price_mismatch_excluded = []
for name, data in stocks_data.items():
    ind = compute_indicators(data["hist"])
    fund = fetch_fundamentals(data["symbol"] + ".CA")

    # فحص اتساق السعر: نقارن آخر سعر إغلاق تاريخي بسعر السوق الحالي من مصدر مستقل (.info).
    # فرق كبير جداً (تقريباً ضعف أو أكثر) غالباً معناه خطأ تعديل تجزئة سهم (stock split) في المصدر - نستبعد السهم
    # بالكامل بدل ما نعرض رقم ممكن يكون غلط، لأن كل المؤشرات الفنية بتتأثر مش السعر بس.
    market_price = fund.get("market_price")
    if market_price and market_price > 0:
        ratio = max(market_price, ind["last_close"]) / min(market_price, ind["last_close"])
        if ratio > 1.9:
            price_mismatch_excluded.append(name)
            continue
...
if price_mismatch_excluded:
    st.markdown(f"""
    <div class="disclaimer">
    🚫 <strong>تم استبعاد {len(price_mismatch_excluded)} سهم من التحليل</strong> بسبب تعارض كبير بين السعر التاريخي وسعر السوق الحالي في مصدر البيانات
    (احتمال خطأ تعديل تجزئة سهم "Stock Split" عند Yahoo Finance) — بدل عرض أرقام قد تكون خاطئة تماماً.
    الأسهم المستبعدة: {", ".join(price_mismatch_excluded)}
    </div>
    """, unsafe_allow_html=True)
